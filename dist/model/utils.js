...
class Attack {
    constructor(name, damage, usageLimit, usageCount = 0) {
        this.name = name;
        this.damage = damage;
...
class Pokeman {
    constructor(name, lifePoint, attacks = []) {
        this.name = name;
...
exports.Pokeman = Pokeman;
class Trainer {
    constructor(name, level = 1, experience = 0, pokemons = []) {
        this.name = name;
        this.level = level;
    ...
    getPokemonWithMostHP() {
        const alivePokemon = this.getAlivePokemons();
        if (alivePokemon.length === 0)
            return null;
        return alivePokemon.reduce((max, current) => current.lifePoint > max.lifePoint ? current : max);
    }
    hasAlivePokemons() {
...
class BattleService {
     ...
     */
    static randomDuel(trainer1, trainer2) {
     ...
     */
    static deterministicDuel(trainer1, trainer2) {
        const t1 = trainer1.clone();
        const t2 = trainer2.clone();
        const battleLog = [];
        let turns = 0;
    ...
    static arena2(trainer1, trainer2) {
        ...
        battleLog.push(`🏆 Arène 2: ${winner.name} remporte le tournoi (${battleCount} combats)!`);
